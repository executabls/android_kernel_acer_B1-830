...
// johann: add global variable to store LCM physical width and height
unsigned  int lcm_physical_width = 0;
unsigned  int lcm_physical_height = 0;
...
//#endif
	...
	case DISP_IOCTL_GET_SESSION_INFO:
	{
		...
		info.physicalHeight = lcm_physical_height;
		info.physicalWidth = lcm_physical_width;
...
static int mtkfb_fbinfo_init(struct fb_info *info)
{
    ...
    lcm_physical_width = var.width  = DISP_GetPhysicalWidth();
    lcm_physical_height = var.height = DISP_GetPhysicalHeight();
