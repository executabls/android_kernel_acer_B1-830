...
#define FULL_CHECK_TIMES		6
 ...
 /* global variable */
 /* ============================================================ // */
  extern kal_uint32 factory_charging_enable;//johnny[2015.5.20]add for factory test
...
kal_uint32 set_bat_charging_current_limit(int current_limit)
{
	...
	/* wake_up_bat(); */
	if(factory_charging_enable == KAL_TRUE){//johnny[2015.6.10],add for factory test
...
{
	CHR_CURRENT_ENUM temp_pro_CC_value = CHARGE_CURRENT_2000_00_MA;//johnny[2015.06.10]termal protection
	...
	if (g_ftm_battery_flag) {
        ...
        if(BMT_status.temperature >= 60 && BMT_status.temperature <= 0)
                temp_pro_CC_value = CHARGE_CURRENT_0_00_MA;
        else if(BMT_status.temperature >= 55 && BMT_status.temperature < 60)
                temp_pro_CC_value = CHARGE_CURRENT_100_00_MA;
        else if(BMT_status.temperature >= 45 && BMT_status.temperature < 55)
                temp_pro_CC_value = CHARGE_CURRENT_500_00_MA;
        else
                temp_pro_CC_value = g_temp_CC_value;

        g_temp_CC_value = min(temp_pro_CC_value,g_temp_CC_value);
        if(temp_pro_CC_value != CHARGE_CURRENT_200_00_MA)
            battery_xlog_printk(BAT_LOG_CRTI, "[johnny] Thermal Protection, set current %d  temp:%d\n",g_temp_CC_value,BMT_status.temperature);
	}
...
#if !defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
#ifdef HIGH_BATTERY_VOLTAGE_SUPPORT
			cv_voltage = BATTERY_VOLT_04_350000_V;//johnny[2014.5.20]modify to 4.35V
#else
			cv_voltage = BATTERY_VOLT_04_200000_V;
#endif
			battery_charging_control(CHARGING_CMD_SET_CV_VOLTAGE, &cv_voltage);
