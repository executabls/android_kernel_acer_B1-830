...
kal_uint32 factory_charging_enable = KAL_TRUE;//johnny[2015.6.10]add for factory test
 /*johnny[2015.6.10]thermal protection*/
 bool thermal_protection = false;
 INT32 pre_charging_state =  0;
  /*johnny[2015.6.10]thermal protection End*/
...
kal_bool g_refresh_ui_soc = KAL_FALSE;//johnny[2015.06.24]add for suspend capacity error
...
/*johnny, implement charging enable/disbale*/
...
{
	char charging_enable_string[10];
	kal_int32 temp_value;
	CHR_CURRENT_ENUM g_temp_input_zero_value = CHARGE_CURRENT_0_00_MA;


	sscanf(buf, "%s", charging_enable_string);

	if(!memcmp(buf,"enable",6)){
            factory_charging_enable = KAL_TRUE;
            temp_value = KAL_FALSE;
            battery_charging_control(CHARGING_CMD_SET_HIZ,&temp_value);
            battery_xlog_printk(BAT_LOG_CRTI, "johnny, charging enable = %d\n",temp_value);
	}
        else if(!memcmp(buf,"disable",7)){
             factory_charging_enable = KAL_FALSE;
             temp_value = KAL_TRUE;
             battery_charging_control(CHARGING_CMD_SET_HIZ,&temp_value);
             battery_xlog_printk(BAT_LOG_CRTI, "johnny, charging enable = %d\n",temp_value);
             //battery_charging_control(CHARGING_CMD_SET_INPUT_CURRENT,&g_temp_input_zero_value);
        }
...
#endif //#ifdef FG_BAT_INT
                        else if(KAL_TRUE == g_refresh_ui_soc)//johnny[2015.06.24]add for suspend capacity error
			{
				BMT_status.UI_SOC--;
				timer_counter = 0;
...
//johnny[2015.06.10]should not keep 1% until low battery voltage
	/*if (BMT_status.UI_SOC <= 0) {
		BMT_status.UI_SOC = 1;
...
void mt_battery_GetBatteryData(void)
{
	...
	{
	BMT_status.temperature =temperature;//johnny[2015.6.10]disable average method
...
void BAT_thread(void)
{
	...
	if(RECOVERY_BOOT == g_platform_boot_mode && 0 == BMT_status.UI_SOC) {//johnny,[2015.06.17], 0% shut down in recovery mode
	    battery_charging_control(CHARGING_CMD_SET_POWER_OFF, NULL);
	    battery_log(BAT_LOG_CRTI," capacity 0 shutdown in recovery mode\n");
	}
...
static int battery_probe(struct platform_device *dev)
{
	...
	/* For EM */
	{
		...
		ret_device_file =
		    ...
		    ret_device_file = device_create_file(&(dev->dev), &dev_attr_Charging_Enable);//johnny,[2015.6.10]
...
static void battery_timer_resume(void)
{
	...
	battery_charging_control(CHARGING_CMD_GET_IS_PCM_TIMER_TRIGGER, &is_pcm_timer_trigger);
        ...
        if(bat_spm_timeout)//johnny[2015.06.24]add for suspend capacity error
